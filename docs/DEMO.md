...
> 자기 랜딩에 적용하고, 검사 5/5 결과로 영수증을 체인에 남긴다. **같은 파일·같은 프롬프트를 검사 도구 없이 돌린 3회는 첫 수정 1/5 · 0/5 · 1/5, 최종은 셋 다 4/5 에서 멈췄다.**
...
| **110–135** | D 탭3 (비교) | `3` 키. 왼쪽 첫 수정 1/5·0/5·1/5 → 최종 4/5 세 번 → 오른쪽 LIVE 5/5 → 적용표(step → 선택자) | "같은 파일, 같은 프롬프트, 팩만 없이 세 번: 첫 수정은 1, 0, 1 이고 최종은 셋 다 4/5 에서 멈췄습니다. 셋 다 놓친 항목이 같습니다.<br>(화면에 타일이 6장이면 위 3장은 `검사 도구 있음` 대조군이다. **먼저 짚고 넘어간다**: "채점기를 쥐여준 세 번은 결국 5/5 를 맞췄습니다. 다만 176~226초, 여러 턴이 걸렸습니다. 아래 세 번이 채점기 없이 돌린, 실제 사용자에 가까운 쪽입니다.") 한국어 제목이 세 줄로 떨어지는 `h1-lines` 입니다. 팩을 산 뒤엔 첫 시도에 5/5, **어느 단계의 교훈이 이 파일의 어느 선택자로 갔는지** 표로 남습니다. 검사 5항목은 판매자가 manifest 에 선언한 보증입니다." | LIVE 가 아직 없거나 5/5 가 아니면 **`R` 키** → 오른쪽이 fallback(사전 실행)으로 바뀌고 "사전 실행" 라벨이 붙는다. 라벨을 가리지 말고 "리허설 기록입니다" 라고 말한다 |
...
- [ ] 현재 배포 (2026-09-13 온체인 재확인): 패키지 `0x50cd511c24786aa091e26a46d5c66ec32308ceb6379902eaf1045d99548f5196` · 판매자 `0xb31cf4c4…560f` · 구매자 `0x40648673…e5a6` · 예비 `0xf4f552bd…992a`. 발행된 팩은 셋이다:
  - **디자인 과정 팩** `0x75b25d24377a92fd976d7690ed73b7b31496c5c4d3b13c96720cf00222cfcb1d` — 5단계, 0.05 SUI, 7일, 미리보기 3건(manifest + 전·후 스크린샷), 구독자 1. 이 대본의 본 시나리오
  - **Sui 기억 팩** `0xaa3b7edcbc7281896372c43a3ca8eae75f3b20accba985af9f4622bc36b52a40` — 30건, 0.01 SUI, 24시간. 텍스트 팩이라 manifest 가 없고 미리보기 2건은 기억 샘플 자체다. 150초 폐기 장면에서 쓴다
  - **한국어 랜딩 카피 팩** `0x8e366e409998682402364c08822d62e2e84639113c712223c0c897521ecde683` — 8단계, 0.03 SUI, 7일, 구독자 0. manifest 는 있으나 스크린샷이 없어 `previews.before/after` 가 둘 다 null 이다. 대신 검사 6항목을 manifest 에 직접 싣고 온다(`tools/check-copy.mjs` 와 같은 목록). 탭2 를 이 팩으로 띄우면 필름스트립이 와이어프레임으로 그려진다
...
- [ ] **탭2·탭3 이 한 팩으로 맞는지** — 9/13 14:30 기준 `demo/state/compare-state.json` 은 **팩 헤더·필름스트립이 한국어 카피 팩(8단계, 썸네일 전부 null → 와이어프레임)인데 live·baseline 은 디자인 팩 리허설 값**이라 섞여 있다. 디자인 팩으로 발표하려면 `npm run mm -- --project C:\demo\seller state` 를 다시 돌려 덮는다. 탭3 하단 검사 칩(`cta-contrast` 등 5개)은 `compare.html` 에 하드코딩이라 카피 팩으로는 맞지 않는다
...
- [ ] **(차단 사항 · 9/13 온체인 재확인) 만료 전에는 BUYER 로 디자인 팩 데모를 돌리지 않는다.** 구매자 지갑 `0x40648673…e5a6` 의 디자인 팩 구독 `0x5deee9ce…d051c` 은 **2026-09-14 23:14:31 KST 만료**이고, 그 구독권으로 **이미 영수증을 남겼다**(팩 A 의 `ReceiptKey{0x5deee9ce…}` 확인, outcome 2). 구독권 1개당 영수증 1회이므로 만료 전에 BUYER 로 돌리면 50–75 결제 장면에 subscribe tx 가 안 뜨고, **135초 영수증 장면이 `EReceiptExists`(abort 5)로 실패한다**(테스트넷 시뮬레이션으로 확인). 만료 전에 찍어야 하면 **예비 지갑 `0xf4f552bd…992a`(BUYER2, 구독 0건 · 0.598 SUI)** 를 `~/.memory-market/config.json` 또는 `.env` 의 BUYER 키로 쓴다. 대안은 구독·영수증이 0건인 **한국어 랜딩 카피 팩**(`0x8e36…`)으로 돌리는 것이다. 9/13 녹화처럼 그 전에 찍어야 하면 예비 지갑 `0xf4f552bd…992a`(BUYER2) 를 `~/.memory-market/config.json` 또는 `.env` 의 BUYER 키로 쓴다. 데모데이(9/19)에는 만료돼 있어 그대로 된다
...
| 지금 시장에 팩이 몇 개 있나 | testnet 에 셋. 디자인 과정 팩(5단계·스크린샷 있음), Sui 기억 팩(30건·텍스트), 한국어 랜딩 카피 팩(8단계·스크린샷 없음, 검사 6항목을 팩이 싣고 옴). ttl 5분 미만인 e2e 테스트 팩은 `market_find` 목록에서 자동으로 빠진다. 세 팩의 모양이 다 달라서 도구가 한 형태에만 맞춰져 있지 않다는 증거가 된다. |
