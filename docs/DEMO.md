...
- [ ] (실측 메모) 구매자 지갑 `0x40648673…e5a6` 의 디자인 팩 구독 `0x5deee9ce…d051c` 은 **2026-09-14 23:14:31 KST 에 만료된다** (9/13 14:24 온체인 확인, 아직 유효). 그때까지는 구매자 지갑으로 돌려도 subscribe tx 가 안 뜬다. 9/13 녹화처럼 그 전에 찍어야 하면 예비 지갑 `0xf4f552bd…992a`(BUYER2) 를 `~/.memory-market/config.json` 또는 `.env` 의 BUYER 키로 쓴다. 데모데이(9/19)에는 만료돼 있어 그대로 된다
